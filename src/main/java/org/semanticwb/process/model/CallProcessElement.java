...
public class CallProcessElement extends org.semanticwb.process.model.base.CallProcessElementBase {
	 ...
	 * @param base
	 */
	public CallProcessElement(SemanticObject base) {
		super(base);
	}
	...
	public String renderElement(HttpServletRequest request, SemanticObject obj, SemanticProperty prop, String propName, String type, String mode, String lang) {
		SemanticObject _obj = obj;
		
		if (_obj == null) {
			_obj = new SemanticObject();
		}
		...
		if (prop.isObjectProperty()) {
			...
			if (aux != null) {
				...
				val = _obj.getObjectProperty(prop);
			}
			...
			if (val != null) {
				uri = val.getURI();
				value = _obj.getDisplayName(lang);
			}
			...
			if (mode.equals("edit") || mode.equals("create")) {
				...
				if (isGlobalScope()) {
					if (cls != null) {
						...
						it = SWBComparator.sortSemanticObjects(lang, SWBPlatform.getSemanticMgr().getVocabulary().listSemanticClassesAsSemanticObjects());
					}
				...
				if (it != null) {
					while (it.hasNext()) {
						...
						if (go instanceof Callable && ((Callable) go).isCallable() && sob.getURI() != null) {
