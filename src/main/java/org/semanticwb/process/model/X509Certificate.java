	...
	static {
		swp_X509File.registerObserver(new SemanticObserver() {
			...
			private void setPropertiesFromCertFile(X509Certificate crt) throws IOException, GeneralSecurityException {
				try (FileInputStream fis = new FileInputStream(SWBPortal.getWorkPath() + crt.getWorkPath() + "/" + crt.getFile())) {
					try (BufferedInputStream bis = new BufferedInputStream(fis)) {
						...
						if (bis.available() > 0) {
							...
							crt.setSerial(getOID25445fromDN(x509.getSubjectDN().getName()));
			...
			private String getOID25445fromDN(String DN) {
