...
public class DocumentationResource extends GenericAdmResource {
	...
	public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
		...
		if (suri != null) {
			...
			if (pe != null) {
				...
				String despliege = getResourceBase().getAttribute("despliegue") != null
						? getResourceBase().getAttribute("despliegue")
						: "Edición";
	...
	void doExportDocument(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
		...
		org.semanticwb.process.model.Process pe = (org.semanticwb.process.model.Process) SWBPlatform.getSemanticMgr()
				.getOntology().getGenericObject(URLDecoder.decode(request.getParameter("suri")));
		response.setContentType("application/zip");
		response.setHeader("Content-Disposition", "attachment; filename=\"" + pe.getTitle() + ".zip\"");
		String basePath = SWBPortal.getWorkPath() + "/models/" + paramRequest.getWebPage().getWebSiteId() + "/Resource/"
				+ pe.getTitle() + "/";
	...
	public static void createModel(String suri, String basePath) throws IOException {
		...
		if (pe instanceof org.semanticwb.process.model.SubProcess) {
			html += "var obj = Modeler.getGraphElementByURI(null, \"" + suri + "\");\n"
					+ "console.log('suri on only model : ' + " + suri + ");" + "ToolKit.setLayer(obj.subLayer);";
		}
		html += "}\n";
	...
	public org.w3c.dom.Document getDom(String suri, javax.servlet.http.HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException, TransformerException {
		...
		if (!suri.equals("")) {
			ProcessElement pe = (ProcessElement) SWBPlatform.getSemanticMgr().getOntology()
					.getGenericObject(URLDecoder.decode(suri));
			...
			String activityT = paramRequest.getLocaleString("activity") != null
					? paramRequest.getLocaleString("activity")
					: "Activity";
			String gatewayT = paramRequest.getLocaleString("gateway") != null ? paramRequest.getLocaleString("gateway")
					: "Gateway";
			String eventT = paramRequest.getLocaleString("event") != null ? paramRequest.getLocaleString("event")
					: "Event";
