...
/**
 * Componente de sistema que permite crear una instancia de procesos.
 *
...
public class CreateProcessInstance extends GenericAdmResource {
	...
	public void doShowMenu(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest)
		...
		SWBResourceURL createUrl = paramRequest.getActionUrl().setAction(ACT_CREATE);
		...
		if (showGroups) {
			...
			if (keys.hasNext()) {
				...
				while (keys.hasNext()) {
					...
					out.println("    <ul>");
					Iterator<Process> itPccs = SWBComparator.sortByDisplayName(groups.get(key).iterator(), lang);
					while (itPccs.hasNext()) {
						Process pcc = itPccs.next();
			...
			if (!processes.isEmpty()) {
				...
				out.println("    <ul>");
				Iterator<Process> itPccs = SWBComparator.sortByDisplayName(processes.iterator(), lang);
				while (itPccs.hasNext()) {
					Process pcc = itPccs.next();
					createUrl.setParameter("prid", pcc.getId());
					out.println("<li><a href=\"" + createUrl + "\">" + pcc.getTitle() + "</a></li>");
