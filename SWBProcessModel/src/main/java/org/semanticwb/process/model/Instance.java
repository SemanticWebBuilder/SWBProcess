 ...
 * de todas las instancias de los nodos de flujo y almacena información del
 * estado de la ejecución de cada elemento configurado de un diagrama BPMN.
 */
public class Instance extends org.semanticwb.process.model.base.InstanceBase {
	...
	/**
	 * Other BPMN statuses to implement: STATUS_READY STATUS_ACTIVE STATUS_INACTIVE
	 * STATUS_COMPLETING STATUS_WITHDRAWN STATUS_COMPLETED STATUS_COMPENSATING
	 * STATUS_FAILING STATUS_TERMINATING STATUS_COMPENSATED STATUS_FAILED
	 * STATUS_TERMINATED STATUS_CLOSED
	 */
	...
	public ProcessInstance getProcessInstance() {
		if (this instanceof FlowNodeInstance)
			return ((FlowNodeInstance) this).getProcessInstance();
		else
			return (ProcessInstance) this;
	}

	/**
	 * Devuelve el tipo del nodo de flujo en el modelo asociado a la instancia del
	 * objeto. Por ejemplo, si el objeto es una tarea de usuario, el método
	...
	public ProcessElement getProcessElementType() {
		...
		if (this instanceof FlowNodeInstance)
			ret = ((FlowNodeInstance) this).getFlowNodeType();
		else
			ret = ((ProcessInstance) this).getProcessType();
		return ret;
	}
	...
	/**
	 * Cierra la instancia del nodo de flujo y continúa al siguiente nodo. Por
	 * defecto asigna el estado CLOSED a la instancia con la acción ACCEPT.
	 * 
	 * @param user
	 *            Usuario que cierra la instancia.
	 */
	...
	/**
	 * Cierra la instancia del nodo de flujo y continúa al siguiente nodo. Asigna la
	 * acción de cierre especificada en el parámetro 'action'.
	 * 
	 * @param user
	 *            Usuario que cierra la instancia.
	 * @param action
	 *            Acción de cierre de la instancia.
	 */
	...
	/**
	 * Cierra la instancia del nodo de flujo marcándola como abortada y continúa al
	 * siguiente nodo. Asigna el estado ABORTED a la instancia con la acción CANCEL.
	 * 
	 * @param user
	 *            Usuario que aborta la instancia.
	 */
	...
	/**
	 * Cierra la instancia del nodo de flujo y continúa al siguiente nodo. Asigna la
	 * información de cierre asociada con los parámetros.
	 * 
	 * @param user
	 *            Usuario que cierra la instancia.
	 * @param status
	 *            Estado asignado a la instancia al momento del cierre.
	 * @param action
	 *            Acción asignada a la instancia al momento del cierre.
	 */
	...
	/**
	 * Elimina las instancias de objetos de datos temporales de una instancia de
	 * proceso.
	 */
	protected void removeTemporallyDataobjects() {
		...
		while (it.hasNext()) {
			...
			if (itemAwareReference.getItemAware() instanceof Collectionable
					|| itemAwareReference.isItemAwareTemporal()) {
	...
	/**
	 * Método que se invoca cuando el nodo de flujo se activa, es decir, cuando el
	 * nodo anterior invoca su método close. Por defecto, el método incrementa el
	 * contador de ejecuciones de la instancia.
	 * 
	 * @param user
	 *            Usuario que ejecuta la instancia.
	 */
	...
	/**
	 * Método que procesa la notificación de un evento propagado por el
	 * ProcessObserver.
	 * 
	 * @param from
	 *            Instancia que genera el evento.
	 */
	...
	/**
	 * Obtiene un iterador a los objetos de datos relacionados con la instancia del
	 * nodo de flujo y sus instancias padres. Es decir, si la instancia es una
	 * actividad en un subproceso, también se recuperan los objetos de datos del
	 * subproceso padre.
	 * 
	 * @return
	 */
	...
	public void setOwnerProperty(SemanticProperty prop, Object value) {
		...
		if (prop.isDataTypeProperty()) {
			...
			if (value instanceof SemanticObject)
				getSemanticObject().setObjectProperty(prop, (SemanticObject) value);
			else if (value instanceof GenericObject)
				getSemanticObject().setObjectProperty(prop, ((GenericObject) value).getSemanticObject());
		}
	}
