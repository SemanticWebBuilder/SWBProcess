...
<%!
    public boolean isAdminUser(User user, WebSite site) {
...
<%
    ...
    int i = 1;
    while(!base.getAttribute(UserTaskInboxResource.ATT_COLS+i, "").isEmpty()) {
    ...
    if (!user.isSigned()) {
        if (paramRequest.getCallMethod() == SWBParamRequest.Call_CONTENT) {
            ...
            <div class="alert alert-block alert-danger fade in">
                <h4>
                    <span class="fa fa-ban"></span> <%=paramRequest.getLocaleString("msgNoAccessTitle")%>
                </h4>
                <p>
                    <%=paramRequest.getLocaleString("msgNoAccess")%>
                </p>
                <p>
                    <a href="/login/<%=site.getId()%>/<%=paramRequest.getWebPage().getId()%>"
                       class="btn btn-default"><%=paramRequest.getLocaleString("btnLogin")%></a>
                </p>
            </div>
            <%
        }
        ...
        if (paramRequest.getCallMethod() == SWBParamRequest.Call_STRATEGY) {
            %>
            <a href="<%=createPiUrl%>" class="btn btn-sm btn-success swbp-btn-start"
               data-toggle="modal" data-target="#modalDialog">
                <span class="fa fa-play-circle fa-lg fa-fw"></span> <%=paramRequest.getLocaleString("createCase")%>
            </a>
            ...
            <div class="row no-margin swbp-button-ribbon text-right">
                <a href="<%=createPiUrl%>" class="btn btn-swbp-action"
                   data-toggle="modal" data-target="#modalDialog">Iniciar proceso</a>
            </div>
            <hr>
            <div class="row form-group swbp-filters">
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12 ">
                    ...
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-padding">
                        ...
                        <select class="form-control"
                                onchange="loadPageUrl('<%=optsUrl%>', 'pf', this.options[this.selectedIndex].value)">
                            <option value="" <%=pFilter.equals("")?"selected":""%>>
                                <%=paramRequest.getLocaleString("allProcesses")%>
                            </option>
                            <%
                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">
                    ...
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-padding">
                        ...
                        %>
                        <select class="form-control"
                                onchange="loadPageUrl('<%=optsUrl%>&sort='+this.options[this.selectedIndex].value, 'pf', this.options[this.selectedIndex].value)">
                            <optgroup label="<%=paramRequest.getLocaleString("sortDate")%>">
                                <option value="1" <%=sortType.equals("1")?"selected":""%>>
                                    <%=paramRequest.getLocaleString("sortLatest")%>
                                </option>
                                <option value="2" <%=sortType.equals("2")?"selected":""%>>
                                    <%=paramRequest.getLocaleString("sortOldest")%>
                                </option>
                            </optgroup>
                            <optgroup label="<%=paramRequest.getLocaleString("sortProcess")%>">
                                <option value="3" <%=sortType.equals("3")?"selected":""%>>
                                    <%=paramRequest.getLocaleString("sortNameAsc")%>
                                </option>
                                <option value="4" <%=sortType.equals("4")?"selected":""%>>
                                    <%=paramRequest.getLocaleString("sortNameDes")%>
                                </option>
                            </optgroup>
                        </select>
                    </div>
                ...
                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-6">
                    ...
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 no-padding">
                        ...
                        %>
                        <select class="form-control"
                                onchange="loadPageUrl('<%=optsUrl%>&sf='+this.options[this.selectedIndex].value, 'pf', this.options[this.selectedIndex].value)">
                            <option <%=sFilter.equals(String.valueOf(FlowNodeInstance.STATUS_PROCESSING))?"selected":""%>
                                    value="<%=String.valueOf(FlowNodeInstance.STATUS_PROCESSING)%>">
                                <%=paramRequest.getLocaleString("pendingTasks")%>
                            </option>
                            <option <%=sFilter.equals(String.valueOf(FlowNodeInstance.STATUS_CLOSED))?"selected":""%>
                                    value="<%=String.valueOf(FlowNodeInstance.STATUS_CLOSED)%>">
                                <%=paramRequest.getLocaleString("closedTasks")%>
                            </option>
                            <option <%=sFilter.equals(String.valueOf(FlowNodeInstance.STATUS_ABORTED))?"selected":""%>
                                    value="<%=String.valueOf(FlowNodeInstance.STATUS_ABORTED)%>">
                                <%=paramRequest.getLocaleString("abortedTasks")%>
                            </option>
                        </select>
                    </div>
                </div>
                                ...
                                <div class="panel-footer">
                                    <div class="row no-margin">
                                        ...
                                        if (instance.getStatus() == ProcessInstance.STATUS_PROCESSING) {
                                            if (isAdminUser(user, site)) {
                                                SWBResourceURL claimUrl = paramRequest.getActionUrl().setAction(UserTaskInboxResource.ACT_CLAIM);
                                                %>
                                                <a class="btn btn-default col-xs-3" href="<%=claimUrl%>?suri=<%=instance.getEncodedURI()%>" <% if (instance.getAssignedto() != null && !user.getURI().equals(instance.getAssignedto().getURI())) {%>onclick="return (confirm('La tarea le ser� asignada y desaparecer� de la bandeja del due�o. �Desea continuar?'))"<%}%>>
                                            ...
                                            if (allowForward) {
                                                ...
                                                } else if (instance.getAssignedto().getURI().equals(user.getURI()) || isAdminUser(user, site)) {
                                                    %>
                                                    <a href="<%=forward.setParameter("suri", instance.getURI())%>" class="btn btn-default col-xs-3" data-toggle="modal" data-target="#modalDialog">
                                                        <span class="col-lg-4 col-md-4 col-sm-12 col-xs-12 fa fa-exchange"></span>
                                                        <span class="col-lg-8 col-md-8 hidden-sm hidden-xs no-padding text-left"><%=paramRequest.getLocaleString("actFwd")%></span>
